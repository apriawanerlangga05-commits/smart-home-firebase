...
import { Mic, MicOff, BookOpen, Headset, ChevronDown } from "lucide-react";
...
export default function VoiceVisualizer({
  ...
  return (
    <div className="bg-gradient-to-br from-[#0c0f1e] to-[#0e1627] border border-gray-800/60 rounded-3xl p-5 sm:p-6 shadow-2xl relative overflow-hidden">
      ...
      {/* Top Header info */}
      <div className="flex flex-col items-center justify-center mb-5 gap-2 text-center">
        <div className="flex items-center gap-2 justify-center">
          <div className="p-1.5 bg-cyan-500/10 rounded-lg text-cyan-400 border border-cyan-500/15">
            <Headset className="w-4 h-4" />
          </div>
          <h3 className="font-bold text-white text-sm tracking-wide font-sans">Asisten Suara Pintar IoT</h3>
        </div>
        <span className={`text-[9.5px] font-mono px-3 py-0.5 rounded-full font-bold tracking-wider transition-all duration-300 ${
          isListening 
      ...
      <div className="flex flex-col items-center justify-center gap-3 bg-[#05070e]/40 border border-gray-900 p-4 rounded-2xl">
        <div className="relative cursor-pointer" onClick={toggleSpeechRecognition}>
          ...
          <button className={`w-14 h-14 rounded-full flex items-center justify-center border transition-all cursor-pointer outline-none ${
            isListening 
      ...
      {/* Collapsible Handbook Content within the Visualizer */}
      <div className="mt-5 pt-4 border-t border-gray-900/60">
        <button
          onClick={() => setIsGuideOpen(!isGuideOpen)}
          className="flex items-center justify-between w-full p-2.5 bg-cyan-500/5 hover:bg-cyan-500/10 rounded-xl border border-cyan-500/10 transition-all text-left outline-none cursor-pointer group"
          id="btn-voice-guide-toggle"
        >
          <div className="flex items-center gap-2">
            <BookOpen className="w-3.5 h-3.5 text-cyan-400 group-hover:scale-110 transition-transform" />
            <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest font-mono">Buku Panduan Perintah Suara</span>
          </div>
          <ChevronDown className={`w-4 h-4 text-cyan-400 transition-transform duration-300 ${isGuideOpen ? "rotate-180" : ""}`} />
        </button>
        ...
        <AnimatePresence initial={false}>
          {isGuideOpen && (
            <motion.div
              ...
              id="voice-guide-content"
            >
              <div className="space-y-2.5 bg-[#05070e] border border-gray-900 rounded-xl p-3 text-[11px] text-gray-400 leading-relaxed font-mono">
                ...
                <div className="flex items-start gap-1.5 border-t border-gray-900/60 pt-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-550 mt-0.5">Mematikan atau menyalakan seluruh relay saklar sekaligus.</p>
                  </div>
                </div>
                <div className="flex items-start gap-1.5 border-t border-gray-900/60 pt-2">
                  <span className="text-cyan-400 mt-0.5 select-none">✦</span>
                  <div>
                    ...
                    <p className="text-[10px] text-gray-550 mt-0.5">Membuka loop sekuens demo lampu dekoratif atau memadamkannya.</p>
                  </div>
                </div>
                <div className="flex items-start gap-1.5 border-t border-gray-900/60 pt-2">
                  <span className="text-cyan-400 mt-0.5 select-none">✦</span>
                  <div>
                    ...
                    <p className="text-[10px] text-gray-550 mt-0.5">Mendengarkan laporan audio status suhu & kelembaban dari sistem.</p>
                  </div>
                </div>
              </div>
            </motion.div>
